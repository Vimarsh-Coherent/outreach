...
async def reorder_steps(
    ...
    await session.flush()
    # Phase 2: assign final orders 1..N
    for new_order, sid in enumerate(ordered_step_ids, start=1):
        rows_by_id[sid].step_order = new_order
    await session.commit()
    return [
        _step_to_out(r) for r in sorted(rows_by_id.values(), key=lambda r: r.step_order)
    ]
