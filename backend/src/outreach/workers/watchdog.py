...
classification backlog.
  ...
  Tier 4 — Deep verify          every 6 hours | Anthropic + OpenAI + Chroma
...
async def _probe_chroma() -> tuple[bool, str]:
    try:
        ...
        client = vault._chroma_client()  # noqa: SLF001
        if client is None:
            return False, "chroma client init failed"
        ef = vault._embedding_fn()  # noqa: SLF001
        if ef is None:
            return False, "no embedding fn (OpenAI key missing?)"
        # Ensure a system collection exists, write+read a sentinel.
        # NOTE: Chroma rejects names starting with non-alphanumeric chars and < 3 chars.
        col = client.get_or_create_collection(
            name="watchdog_probe", embedding_function=ef,
        )
        col.upsert(documents=["watchdog probe"], ids=["sentinel"])
        r = col.query(query_texts=["probe"], n_results=1)
        docs = (r or {}).get("documents") or [[]]
        return len(docs[0]) >= 1, f"got {len(docs[0])} doc(s)"
    except Exception as e:  # noqa: BLE001
        return False, f"{type(e).__name__}: {str(e)[:80]}"
...
async def deep_verify() -> dict:
    ...
    return {
        ...
        "chroma":    {"ok": c_ok, "detail": c_detail},
    }
