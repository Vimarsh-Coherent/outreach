...
function EditLeadModal({ lead, onClose, onSaved }: { lead: LeadOut; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<Record<LeadField, string>>({
    email: lead.email ?? "",
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      const payload = Object.fromEntries(
        LEAD_FIELDS.map(f => [f.key, form[f.key].trim() === "" ? null : form[f.key].trim()])
      );
      return updateLead(lead.id, payload);
    },
    onSuccess: () => onSaved(),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b shrink-0">
          <h2 className="font-semibold text-slate-900 text-lg">Edit {leadName}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 text-2xl leading-none">&times;</button>
        </div>
        ...
        <div className="px-6 py-3 border-t shrink-0 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-1.5 rounded border text-sm text-slate-600 hover:bg-slate-50">
            Cancel
          </button>
          <button
            onClick={() => { setError(null); saveMut.mutate(); }}
            disabled={saveMut.isPending || (!form.email.trim() && !form.phone.trim() && !form.linkedin_url.trim())}
            className="px-4 py-1.5 rounded bg-emerald-600 text-white text-sm hover:bg-emerald-700 disabled:bg-slate-300"
          >
    ...
    <div className="space-y-8 max-w-6xl">
      ...
      {editLead && (
        <EditLeadModal
          ...
          onSaved={() => { setEditLead(null); qc.invalidateQueries({ queryKey: ["leads"] }); }}
        />
      )}
                ...
                <tbody>
                  {leadsData.items.map(l => (
                    <tr key={l.id} className="border-t">
                      ...
                      <td className="py-2">
                        <div className="flex items-center gap-3">
                          <button onClick={() => setEditLead(l)} className="text-sky-600 hover:underline text-xs">edit</button>
